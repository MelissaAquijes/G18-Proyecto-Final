...
import {getEmailInput, getPasswordInput, getPassword2Input,
        getNameInput, getLastnameInput} from "../../../../app/slices/formInputSlice";
...
export default function RegisterFields(){
  ...
  const registerPassword = useSelector((state)=>state.formInputs.passwordInput);
  const registerPassword2 = useSelector((state)=>state.formInputs.password2Input);
  const registerName = useSelector((state)=>state.formInputs.nameInput);
  const registerLastname = useSelector((state)=>state.formInputs.lastNameInput);
  ...
  const handleFormSubmit = (event) => {
    ...
    console.log(registerEmail,registerPassword,registerPassword2,registerName,registerLastname);
  };


  return(

    <form id="registerFields" onSubmit={handleFormSubmit} className="w-full h-[85%] flex flex-col justify-center gap-2 items-center rounded-xl">
      
      <section className="w-full h-[70%] flex flex-col justify-center items-center gap-4 rounded-xl bg-lavender">

          <div id="emailRegisterField" className="w-full h-1/5 flex border-4 rounded-xl">  
          ...
          <div id="pswdFieldRegister" className="h-1/5 w-full flex border-4 rounded-xl">
            <i className="fa-solid fa-key w-[10%] h-full flex justify-center items-center rounded-l-[8px] border-r-4"></i>
            <input type="password" name="pswdRegister" 
          ...
          <div id="confirmPswdField" className="h-1/5 w-full flex border-4 rounded-xl">
      ...
      <section id="submitAllRegister" className="w-full h-[25%] font-bold">
        ...
        <button type="submit" className={handleButtonClass()}>REGISTRARME</button>
      </section>
